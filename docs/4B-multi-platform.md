...
- **QQ 能与 Hhclaw 同机**：NapCat 跑 Docker（WSL 的 Docker），与 Hhclaw 同机，走 OneBot 11 反向 WebSocket。
- **微信必须拆 Windows 原生桥接进程**：wechatferry 注入 PE 格式 DLL，只能在原生 Windows 跑，Hhclaw 在 WSL，两者走 localhost HTTP。
...
| 模式 | 方向 | 评价 |
|------|------|------|
| 正向 WebSocket | Hhclaw 主动连 bot 6700 端口 | 不选 |
| **反向 WebSocket** | **bot 主动连 Hhclaw 的 `/onebot/ws`** | **选定，Hhclaw 只维护一个 WS 服务** |
...
转 Hhclaw 内部消息模型（说明书第 4 章）：

- `text` 段 → 拼接为 `text` 字段。
- `at` 段 → 判断是否 @ 自己（决定应不应答）+ 提取被 @ 人。
- `image` / `file` 段 → 提取 url → 下载到本地 → 转附件字段。
- 反向发送：Hhclaw 统一消息 → 按内容拆 `text` / `image` 段 → 调 `/send_msg` API。
...
结论：**多账号 = 多容器（多实例）**，每号一个 NapCat 容器，各自反向 WS 连到 Hhclaw 的同一个 `/onebot/ws` 端点。

账号区分机制：

- OneBot 握手/事件里带 `self_id`（bot 自己的 QQ 号），Hhclaw 用它作为连接的唯一 key，注册进连接注册表。
...
微信进程 --(DLL hook)--> wcferry 回调 --> 桥接脚本(Windows原生)
                                          ...
                                          ├─ 白名单过滤(只应答目标群/好友)
                                          ├─ 转 Hhclaw 统一消息模型
                                          └─ localhost HTTP 发给 Hhclaw
Hhclaw 回复 --> 桥接脚本 --> wcferry 发送接口 --> 微信被动回复
```

要点：**只被动回复，不主动群发**（风控最低）；桥接脚本跑 Windows 原生，Hhclaw 跑 WSL，两者 localhost 通信。

### 2.3 异常处理（补充点 2）

分三层，自愈边界清晰：

**层级 1：桥接脚本进程崩溃 → Windows 侧自动拉起，Hhclaw 无需介入**

- 用 Windows 任务计划程序（Task Scheduler）或 NSSM 守护桥接脚本，进程退出即自动重启。
- Hhclaw 不直接拉起 Windows 侧进程（跨系统，做不到也不该做）。
...
**层级 3：Hhclaw 侧心跳监测 → 感知失联 + 降级**

- 桥接脚本每 30s 向 Hhclaw 发 `POST /bridge/heartbeat`。
- Hhclaw 超过 90s 未收到心跳 → 将该连接标记 `offline`，投递函数静默/丢弃（个人助手场景不缓存），记日志，**不阻塞其他平台**（QQ 照常）。
- 桥接自愈后重新注册连接 → 恢复 `online`。

设计咬合：这正好落在说明书方案 C 的「连接注册表 + 投递函数抽象」上——投递函数内部封装断线重试；桥接失联时投递返回失败，Hhclaw 标记连接离线，与 QQ 连接同等对待。
...
- Hhclaw 的 Agent 执行器只认「连接 key + 消息内容」，不碰「怎么投」。
